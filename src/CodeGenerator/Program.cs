...
namespace CodeGenerator
{
    internal static class Program
    {
        ...
        {
            var outputPath = args.Length > 0 ? args[0] : AppContext.BaseDirectory;

            var libraryName = args.Length > 1 ? args[1] : "cimgui";

            var projectNamespace = libraryName switch
            {
                "cimgui" => "ImGuiSharp",
                "cimplot" => "ImPlotSharp",
            ...
            var referencesImGui = libraryName switch
            {
                "cimgui" => false,
                "cimplot" => true,
                "cimnodes" => true,
            ...
            var classPrefix = libraryName switch
            {
                "cimgui" => "ImGui",
                "cimplot" => "ImPlot",
                "cimnodes" => "Imnodes",
            ...
            var dllName = libraryName switch
            {
                "cimgui" => "cimgui",
                "cimplot" => "cimplot",
                "cimnodes" => "cimnodes",
            ...
            var definitionsPath = Path.Combine(AppContext.BaseDirectory, "definitions", libraryName);
            ...
            using (var writer = new CSharpCodeWriter(Path.Combine(outputPath, $"{classPrefix}Native.gen.cs")))
            {
                writer.Using("System");
                writer.Using("System.Numerics");
            ...
            using (var writer = new CSharpCodeWriter(Path.Combine(outputPath, $"{classPrefix}.gen.cs")))
            {
                writer.Using("System");
                writer.Using("System.Numerics");
                writer.Using("System.Runtime.InteropServices");
